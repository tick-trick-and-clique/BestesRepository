...
def modular_product(g1, g2):
    ...
    for v1 in g1.get_list_of_vertices():
        g1_cut = g1.get_list_of_vertices().copy()
        g1_cut.remove(v1)
        for v2 in g1_cut:
            for v3 in g2.get_list_of_vertices():
                g2_cut = g2.get_list_of_vertices().copy()
                g2_cut.remove(v3)
                for v4 in g2_cut:
    ...
    print(GRAPH("Modular Product of " + g1.get_name() + " and " + g2.get_name(), new_list_of_vertices,
                  new_list_of_edges, new_number_of_vertices, int(new_number_of_edges/2), False))
